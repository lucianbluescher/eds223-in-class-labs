...
```{r}
# create low spatial template resolution raster
tempalte_raster_low <- rast(ext(park_polygon), resolution = 0.5, crs = st_crs(park_polygon))
...
#| message: false
map1 <- tm_shape(L7) +
  tm_rgb(col = tm_vars(c(3,2,1), multivariate = TRUE)) + tm_title(text = "true color")

map2 <- tm_shape(L7) + 
  tm_rgb(col = tm_vars(c(4,3,2), multivariate = T)) +
  tm_title(text = "NIR, red, green")

map3 <- tm_shape(L7) + 
  tm_rgb(col = tm_vars(c(5,4,2), multivariate = T)) +
...
```{r}
#| message: false
# Define NDVI as the normalized difference between green and NIR bands 
ndwi_fun <- function(green, nir){
  (green - nir) / (green + nir)
}

# apply NDWI function to landsat bands 3 & 4
ndwi_rast <- lapp(L7[[c(3,4)]], fun = ndwi_fun)
...
*Useful for drought monitering and fire risk*
```{r}
...
tmap_arrange(map1, map3, map_ndwi, map_ndmi)
```
