...
# mask raster to the southern portion of SB county
landsat_masked <- mask(landsat_cropped, SB_county_south)

# remove unecessary objects from environment
...
# reclassify eroneous values as NA
...
```{r}
# classify image based on decision tee
...
# Extract spectral bands for clusting
...
landst_df$cluster <- kmeans_result$cluster
```
...
# Take the x,y coordinate from the landsat_df and convert them to cell numbers for the raster
...
```{r}
# Look at cluser characteristics to determine landcover type
landsat_df |> 
  group_by(cluster) |> 
  summarise(
    blue = mean(blue),
