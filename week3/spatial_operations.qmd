...
```{r}
# subset nz_heighs to just the features that intersect Canterbury
...
# Basic left join format
joined_df <- left_join(df_A, df_B, by = c("id_A" = "id=B"))
...
cycle_hire_osm <- pData:: cycle_hire_osm
```
...
incongruent2 <- incongruent |> select(value)
...
map1 <- tm_shape(us_states) +
  ...
  #tm_layout(meta.margins = c(0.1, 0, 0, 0))
