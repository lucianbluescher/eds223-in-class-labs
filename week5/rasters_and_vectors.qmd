...
```{r}
# Cut raster to match extent of boundary vector
...
bryce_small %>% 
  # Mapping colors to elevation values (default dark to light)
  height_shade() %>%  
  # plotting the map
  plot_map() 
```
...
bryce_small %>% 
  ...
  # Emphasize riges by adding textuer shading
...
# Convert the cropped raster to a matrix fro rayshader
...
```{r}
# Set new bounding box using coordinates pulled from OSM
osm_bbox = c(-112.174228, 37.614998, -112.156230,37.629084)

# Querying OSM for all "highway" features (roads, paths, trails)
bryce_highway = opq(osm_bbox) %>% # This boundary
  add_osm_feature("highway") %>% # This feauture
  osmdata_sf() #From tis data
bryce_highway
```
...
# Separate lines into trails, footpatchs and bridleway
...
```{r}
# Transform CRS to match from water lines to bryce crs
bryce_streams = st_transform(bryce_water_lines$osm_lines,crs=crs(bryce)) 

# Create stream layer with streams
...
```{r}
# Create map with each overlay using 
base_map %>% 
