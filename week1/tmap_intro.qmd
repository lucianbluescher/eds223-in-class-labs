...
# country boundaries
world_vector <- read_sf(here::here('data', 'worldvector.gpkg'))

# major cities
world_cities <- read_sf(here::here('data', 'worldvector.gpkg'))
...
```{r}
# plotting vector and raster data on the same map
...
```{r}
tm_shape(world_elevation, bbox = world_cities) + # bounding box = world_cities
  tm_raster() +
tm_shape(world_vector)+
  tm_borders()
...
knitr::kable(attr_layers_df, 
  ...
  kableExtra::kable_styling("striped",
                            latex_options = "striped", 
                            full_width = FALSE) %>% 
  kableExtra::rcolumn_spec(1, bold = TRUE, monospace = TRUE)
```
...
Maps can be stored as objects for for adding additional layers and saving programmatically. Maps can be saved directly in `tmap` using the `tm_save()` functi
